...
from sita.serializers.siem import SIEMSerializer
...
class SIEMViewSet(viewsets.ModelViewSet):
    ...
    def event_count_by_usecases(self, request, *args, **kwargs):
        if request.query_params:
    ...
    def severity_by_usecase(self, request):
        if request.query_params:
            ...
            if usecases:
                            ...
                            if siem_data:
                                for data in siem_data:
                                    ...
                                    if severity_mean > 6:
                                        ...
                                        low += 1
                                        total_events_data['total_low_events'] = total_events_data.get('total_low_events', 0) + data.event_count
                ...
                return Response({
                    "Status": status.HTTP_404_NOT_FOUND,
                    "Message": "You don't have this Usecase"
                })
        else:
            return Response(
                {
    ...
    def avg_res_time(self, request, *args, **kwargs):
        siem_data = SIEMService.get_queryset()
        count = 0
        all_works_done = 0
        if siem_data:
    ...
    def offence_by_usecases(self, request, *args, **kwargs):
        usecases = UseCaseService.get_queryset()
        all_usecase = []
